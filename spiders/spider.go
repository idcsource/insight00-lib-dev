...
func NewSpider(name string, rolec *rcontrol.RolesControl, smcsn *smcs2.NodeSmcs, logs *ilogs.Logs) *Spider {
	...
	sp := &Spider{
		Name:         name,
		RolesControl: rolec,
		NodeStatus:   smcs2.NODE_STATUS_NO_CONFIG,
		WorkStatus:   smcs2.WORK_SET_STOP,
...
func (s *Spider) SmcsNodeOperator(data smcs2.CenterSend) (err error) {
	...
	if data.NewConfig == true {
		spdata := cpool.NewConfigPoolNoFile()
		spdata.DecodePool(data.Config)
		s.Config = spdata
		ifrestart = true
		ifconfig = true
	}
	...
	if ifstatus == true && s.WorkStatus == smcs2.WORK_SET_STOP {
...
func (s *Spider) start(ifstatus, ifconfig bool) {
	...
	if ifstatus == true {
		if s.WorkStatus == smcs2.WORK_SET_START || s.WorkStatus == smcs2.WORK_SET_STOP {
			var oprate uint8
			if s.WorkStatus == smcs2.WORK_SET_START {
	...
	for {
		...
		if runlog != nil {
			for _, one := range runlog {
				s.NodeSMCS.RunLog(one)
			}
		}
		errlog := s.ReErrLog()
		if errlog != nil {
			for _, one := range errlog {
				s.NodeSMCS.ErrLog(one)
			}
		}
