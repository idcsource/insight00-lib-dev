...
这是一种妥协，为了某种更有必要的灵活而放弃了另一种必要性较小的灵活（副作用是空间占用增加）。
...
但是，妥协是必须的。“角色”所支持的数据点格式被限制了，但我想也够用了，包含time.Time、[]byte、string、uint8、uint、uint64、int8、int、int64、float32、float64、complex64、complex128,以及除去time.Time和[]byte外所有这些类型的slice和map[string]类型。

另外，虽然存储空间是可以缩小的，但我没有缩，写起来太麻烦。
